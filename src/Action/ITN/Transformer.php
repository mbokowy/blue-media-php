...
class Transformer
{
     ...
     * @return bool
     */
    private static function isArrayClearanceTransaction(SimpleXMLElement $transaction): bool
    {
        return (
     ...
     * @return bool
     */
    private static function isObjectClearanceTransaction(ItnIn $itnModel): bool
    {
        return (
     ...
     * @return array
     */
    public static function modelToArray(ItnIn $model): array
    {
        $model->validate();
        ...
        if (count($result['customerData']) == 0) {
            unset($result['customerData']);
        }
        if (count($result['recurringData']) == 0) {
            unset($result['recurringData']);
        }
        if (count($result['cardData']) == 0) {
            unset($result['cardData']);
        }
     ...
     * @throws \Exception
     */
    public static function toModel(SimpleXMLElement $xml): ItnIn
    {
